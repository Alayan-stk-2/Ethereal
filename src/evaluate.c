...
const int PawnValue   = S(  87, 128);
const int KnightValue = S( 452, 428);
const int BishopValue = S( 474, 443);
const int RookValue   = S( 659, 713);
const int QueenValue  = S(1299,1386);
...
const int PawnPSQT32[32] = {
    ...
    S( -16,  11), S(   2,   8), S(  -7,   4), S(  -2,  -1), 
    S( -23,   7), S( -16,   3), S( -10,  -9), S(   1, -14), 
    S( -14,   9), S( -12,   9), S(   5, -10), S(  14, -24), 
    S(  -6,  20), S(   2,  13), S(   0,  -5), S(   5, -22), 
    S(  -5,  36), S(   0,  33), S(  11,  20), S(  34, -11), 
    S( -16, -38), S( -65,  -7), S(   3, -22), S(  40, -38), 
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0), 
};

const int KnightPSQT32[32] = {
    S( -49, -25), S( -10, -33), S( -12, -27), S(   2, -17), 
    S(  -1, -19), S(   3, -13), S(   3, -29), S(  17, -19), 
    S(   5, -22), S(  21, -21), S(  14, -12), S(  23,  -3), 
    S(  16,   6), S(  23,   7), S(  32,  16), S(  31,  25), 
    S(  22,  16), S(  24,  12), S(  42,  24), S(  32,  34), 
    S( -16,  14), S(   4,  13), S(  28,  25), S(  30,  27), 
    S(   2, -13), S(  -6,   1), S(  35, -19), S(  38,  -4), 
    S(-168, -17), S( -81,  -2), S(-110,  18), S( -30,   1), 
};

const int BishopPSQT32[32] = {
    S(  21, -17), S(  15, -20), S(  -1, -10), S(  13, -10), 
    S(  31, -31), S(  24, -28), S(  23, -22), S(  10,  -9), 
    S(  14, -14), S(  32, -10), S(  18,  -3), S(  18,   1), 
    S(  21,  -8), S(  13,  -4), S(  21,   6), S(  20,   8), 
    S( -15,  10), S(  13,   5), S(   7,  11), S(  14,  18), 
    S(   3,   6), S(  -1,  14), S(  14,  11), S(  14,   5), 
    S( -45,  13), S( -38,  10), S(  -9,   3), S( -21,   7), 
    S( -41,   2), S( -48,   8), S( -86,  17), S( -91,  24), 
};

const int RookPSQT32[32] = {
    S(  -8, -20), S(  -4, -19), S(  -2, -19), S(   8, -23), 
    S( -42,  -9), S(  -8, -26), S(  -5, -25), S(   7, -24), 
    S( -26, -12), S(  -6, -12), S( -13, -13), S(   2, -19), 
    S( -16,   0), S(  -7,   5), S(  -8,   3), S(   5,  -3), 
    S(  -4,   7), S(  13,   6), S(  21,   1), S(  30,  -6), 
    S( -13,  16), S(  26,   5), S(   2,  17), S(  28,  -3), 
    S(   0,   5), S( -16,  18), S(   8,  10), S(  18,   4), 
    S(  32,  18), S(  23,  21), S(   5,  29), S(  13,  22), 
};

const int QueenPSQT32[32] = {
    S(  24, -52), S(   6, -38), S(  12, -47), S(  19, -39), 
    S(  16, -38), S(  29, -55), S(  31, -64), S(  26, -22), 
    S(  11, -22), S(  30, -17), S(  13,   6), S(  13,   2), 
    S(  14,  -3), S(  14,  18), S(   5,  22), S(  -6,  63), 
    S( -17,  16), S(  -6,  41), S( -12,  22), S( -25,  72), 
    S( -24,  26), S( -20,  17), S( -22,  24), S( -14,  25), 
    S(  -9,  25), S( -55,  65), S( -15,  15), S( -42,  52), 
    S( -10,  15), S(  17,   8), S(   8,   7), S(  -7,  18), 
};

const int KingPSQT32[32] = {
    S(  44, -80), S(  31, -54), S( -14, -18), S( -26, -19), 
    S(  32, -30), S(   1, -22), S( -32,   0), S( -48,   1), 
    S(   9, -32), S(  18, -30), S(  15,  -8), S( -10,   6), 
    S(   2, -35), S(  82, -37), S(  41,   2), S(  -7,  19), 
    S(   4, -20), S(  95, -28), S(  46,  14), S(   2,  26), 
    S(  46, -21), S( 120, -15), S(  94,  10), S(  39,  12), 
    S(   6, -40), S(  47,  -3), S(  32,  13), S(   9,   4), 
...
const int PawnCandidatePasser[2][8] = {
   {S(   0,   0), S( -27, -10), S( -12,   9), S( -11,  29), S(   6,  61), S(  46,  66), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -17,  15), S(  -5,  22), S(   5,  45), S(  21,  81), S(  32,  54), S(   0,   0), S(   0,   0)},
};

const int PawnIsolated = S( -13,  -7);
...
const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0), 
    S(   0,  -8), S(   2,   0), S(   7,  -4), S(  12,  16), 
    S(  10,   0), S(  23,   3), S(  23,  10), S(  23,  18), 
    S(  10,   0), S(  17,   4), S(  11,  10), S(  18,  19), 
    S(  10,   9), S(  18,  13), S(  26,  19), S(  30,  25), 
    S(  55,  25), S(  48,  48), S(  68,  55), S(  84,  60), 
...
const int KnightBehindPawn = S(   6,  17);

const int KnightMobility[9] = {
    S( -74,-104), S( -33, -94), S( -17, -37), S(  -8, -12), 
    S(   6,  -5), S(  10,   7), S(  20,  11), S(  32,  12), 
    S(  46,   3), 
};
...
const int BishopMobility[14] = {
    S( -65,-147), S( -27, -93), S( -15, -52), S(  -6, -23), 
    S(   7, -12), S(  16,  -2), S(  20,   5), S(  23,  11), 
    S(  25,  16), S(  26,  19), S(  24,  19), S(  42,   9), 
    S(  42,  18), S(  71, -13), 
};
...
const int RookOnSeventh = S(  -5,  29);

const int RookMobility[15] = {
    S(-148,-113), S( -52,-113), S( -10, -53), S(   3, -16), 
    S(   1,   3), S(  -4,  18), S(  -3,  27), S(   2,  29), 
    S(   5,  29), S(   7,  34), S(   6,  41), S(  11,  44), 
    S(  13,  46), S(  27,  39), S(  78,  10), 
};
...
const int QueenMobility[28] = {
    ...
    S(  -4,-139), S(   1, -83), S(   5, -52), S(   5, -22), 
    S(   9, -14), S(   9,   7), S(  12,  13), S(  12,  26), 
    S(  14,  25), S(  14,  31), S(  14,  36), S(  14,  41), 
    S(  10,  43), S(   8,  46), S(   4,  42), S(   2,  37), 
    S(  10,  18), S(  22,  -6), S(  28, -33), S(  30, -51), 
...
const int KingDefenders[12] = {
    S( -24,  -9), S( -15,   4), S(  -5,   4), S(   8,   3), 
    S(  21,   5), S(  35,   6), S(  32,  -2), S(  13,   0), 
    S(  12,   6), S(  12,   6), S(  12,   6), S(  12,   6), 
};

const int KingPawnFileProximity[8] = {
    S(  46,  25), S(  -1,  25), S(   2,  14), S( -14, -18), 
    S( -16, -47), S( -14, -61), S( -14, -66), S( -11, -70), 
};

const int KingShelter[2][8][8] = {
  {{S( -12,   3), S(  17, -24), S(  17, -10), S(  12,   4), S(   6,   4), S(   1,   2), S(  -3, -33), S( -46,  17)},
   {S(  16,  -8), S(  20, -19), S(  -1,  -5), S( -15,   4), S( -30,  15), S( -70,  68), S(  92,  82), S( -28,  -1)},
   {S(  35,  -5), S(  12, -10), S( -27,   6), S( -10,  -8), S( -19,  -4), S( -11,   1), S(   0,  66), S( -11,  -1)},
   {S(   3,   9), S(  16, -10), S(   0, -11), S(  13, -21), S(  21, -35), S( -58,   5), S(-136,  52), S(  10,  -7)},
   {S( -13,   8), S(   5,  -5), S( -23,  -1), S( -16,   4), S( -19,  -6), S( -41,  -1), S(  33, -17), S(  -5,  -4)},
   {S(  43, -13), S(  14, -14), S( -25,  -1), S(  -4, -15), S(   6, -24), S(  18, -21), S(  41, -30), S( -25,  -1)},
   {S(  26, -12), S(   7, -15), S( -21,  -3), S( -17,  -8), S( -29,  -7), S( -36,  32), S(   0,  44), S( -13,   0)},
   {S(  -6,  -9), S(   6, -14), S(   7,  -1), S(  -3,   8), S( -11,  14), S(  -8,  37), S(-190,  87), S( -25,  13)}},
  {{S(   0,   0), S(  -9, -23), S(   4, -18), S( -40,  16), S( -22,   2), S(   3,  42), S(-167,  -8), S( -45,   6)},
   {S(   0,   0), S(  23, -21), S(   7,  -7), S( -19,  -1), S(  -1, -12), S(  26,  66), S(-184,  -3), S( -38,   3)},
   {S(   0,   0), S(  29, -15), S(  -4,  -8), S(   4, -17), S(  14,  -8), S( -87,  47), S( -84, -73), S( -22,  -2)},
   {S(   0,   0), S(  -3,   5), S(  -3,  -1), S( -17,   1), S( -27,   1), S( -99,  31), S(   7, -41), S( -19,  -2)},
   {S(   0,   0), S(   8,   1), S(  11,  -5), S(  16, -11), S(  14, -26), S( -57,  15), S(-104, -61), S(  -1,  -2)},
   {S(   0,   0), S(  10, -10), S( -18,  -1), S( -25,  -7), S(  17, -24), S( -38,   3), S(  55,  38), S( -17,  -6)},
   {S(   0,   0), S(  23, -15), S(  15,  -9), S( -15,  -9), S( -26,   8), S(  -8,  15), S( -56, -49), S( -38,  11)},
   {S(   0,   0), S(  13, -36), S(  21, -24), S( -19,  -5), S( -17,  18), S(  -5,  20), S(-228, -55), S( -23,  -1)}},
};

const int KingStorm[2][4][8] = {
  {{S(  -4,  28), S( 118,  -8), S( -24,  21), S( -21,   8), S( -17,   3), S( -11,   0), S( -11,   5), S( -27,   2)},
   {S(  -3,  46), S(  57,  11), S( -20,  17), S(  -8,   6), S(  -3,   3), S(   4,  -2), S(   2,   3), S( -14,  10)},
   {S(   8,  38), S(  17,  24), S( -23,  15), S( -14,   5), S(  -1,   3), S(   9,  -2), S(   7,  -3), S(   3,   7)},
   {S(  -3,  21), S(  16,  17), S( -17,   8), S( -18,   1), S( -11,   2), S(   6,  -9), S(  -1,  -9), S( -13,   9)}},
  {{S(   0,   0), S( -15, -16), S( -16,  -2), S(  19, -17), S(  11,  -7), S(   2, -18), S(  -2,   0), S(  17,  29)},
   {S(   0,   0), S( -16, -34), S(   0,  -8), S(  34, -13), S(  -1,  -2), S(  14, -23), S(  -6, -10), S( -17,   2)},
   {S(   0,   0), S( -28, -49), S( -27, -10), S(  11, -11), S(   3,  -2), S(  -7, -14), S( -12, -15), S( -11,   6)},
   {S(   0,   0), S(  -2, -18), S( -15, -20), S(  -7,  -5), S(   0,  -6), S(   4, -24), S(  71, -11), S(  13,  20)}},
};
...
const int PassedPawn[2][2][8] = {
  {{S(   0,   0), S( -38,   4), S( -56,  22), S( -80,  27), S(  -7,  13), S(  70,  -4), S( 157,  57), S(   0,   0)},
   {S(   0,   0), S( -29,   7), S( -49,  25), S( -73,  31), S( -11,  34), S(  89,  30), S( 182, 100), S(   0,   0)}},
  {{S(   0,   0), S( -25,  16), S( -51,  23), S( -73,  38), S(  -3,  36), S(  88,  39), S( 262, 117), S(   0,   0)},
   {S(   0,   0), S( -28,  10), S( -46,  14), S( -67,  38), S(  -1,  58), S(  94, 127), S( 162, 278), S(   0,   0)}},
};

const int PassedFriendlyDistance[8] = {
    S(   0,   0), S(  -1,   2), S(   4,  -4), S(  11, -10), 
    S(   6, -15), S( -11, -18), S( -18, -12), S(   0,   0), 
};

const int PassedEnemyDistance[8] = {
    S(   0,   0), S(   6,  -3), S(   6,   1), S(   7,   9), 
    S(   2,  26), S(   7,  41), S(  24,  39), S(   0,   0), 
};

const int PassedSafePromotionPath    = S( -27,  49);

/* Threat Evaluation Terms */

const int ThreatWeakPawn             = S(   4, -31);
const int ThreatMinorAttackedByPawn  = S( -58, -47);
const int ThreatMinorAttackedByMinor = S( -25, -28);
const int ThreatMinorAttackedByMajor = S( -24, -46);
const int ThreatRookAttackedByLesser = S( -50, -14);
const int ThreatMinorAttackedByKing  = S( -21, -22);
const int ThreatRookAttackedByKing  = S( -15, -22);
const int ThreatQueenAttackedByOne  = S( -54, -32);
const int ThreatOverloadedPieces    = S(  -9, -13);
const int ThreatByPawnPush          = S(  21,  25);

/* Closedness Evaluation Terms */

const int ClosednessKnightAdjustment[9] = {
    S( -13,  -9), S( -10,   7), S(  -7,  14), S(  -3,  16), 
    S(  -2,  21), S(   3,  18), S(   5,  14), S(  -4,  28), 
    S(  -4,  17), 
};

const int ClosednessRookAdjustment[9] = {
    S(  46,  12), S(   8,  26), S(   6,  17), S(  -1,   6), 
    S(  -4,   5), S( -10,  -8), S( -13, -11), S( -21, -15), 
    S( -26, -16), 
};
