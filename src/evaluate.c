...
const int KnightValue = S( 822, 767);
const int BishopValue = S( 862, 802);
const int RookValue   = S(1228,1298);
const int QueenValue  = S(2397,2449);
...
const int PawnPSQT32[32] = {
    ...
    S( -54, -19), S( -18, -24), S( -37, -19), S( -27, -26), 
    S( -56, -24), S( -44, -21), S( -37, -32), S( -21, -49), 
    S( -50, -13), S( -49,  -9), S(  -2, -47), S( -10, -67), 
    S( -33, -11), S( -28, -15), S( -30, -40), S( -11, -66), 
    S( -41,  19), S( -37,  11), S( -36,   2), S(  18, -47), 
    S( -50,-132), S(-132, -71), S(  -8, -96), S(  74,-121), 
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0), 
};

const int KnightPSQT32[32] = {
    S( -92, -60), S( -32, -70), S( -41, -62), S( -14, -40), 
    S( -22, -46), S( -15, -36), S( -14, -66), S(  -2, -45), 
    S(  -8, -63), S(  13, -59), S(   3, -44), S(  25, -26), 
    S(   5,  -8), S(  14,  -9), S(  27,  10), S(  32,  21), 
    S(  17,   8), S(  22,  -4), S(  38,  26), S(  23,  49), 
    S( -59,   5), S( -33,   3), S(  22,  28), S(  16,  35), 
    S( -19, -41), S( -55, -27), S(   1, -58), S(  58, -35), 
    S(-380, -53), S(-164, -15), S(-230,  14), S( -72, -28), 
};

const int BishopPSQT32[32] = {
    S(   8, -47), S(  -9, -44), S( -44, -33), S( -10, -36), 
    S(  23, -76), S(  11, -69), S(   7, -53), S(  -8, -39), 
    S(  -3, -38), S(  19, -46), S(   1, -34), S(  11, -32), 
    S(  -5, -28), S(   0, -23), S(  -3, -16), S(  11, -11), 
    S( -56,   9), S(   1, -11), S( -27,   4), S( -18,  10), 
    S( -29,  -4), S( -40,  13), S( -23,  10), S( -13,   5), 
    S(-113,  14), S(-115,  10), S( -59,  -4), S( -75,   3), 
    S(-110,  -7), S( -98,  -2), S(-186,  13), S(-187,  32), 
};

const int RookPSQT32[32] = {
    S( -64, -91), S( -70, -76), S( -48, -84), S( -36, -91), 
    S(-133, -63), S( -61,-101), S( -53,-100), S( -33,-106), 
    S( -91, -68), S( -53, -62), S( -66, -74), S( -36, -90), 
    S( -76, -53), S( -63, -38), S( -52, -53), S( -29, -60), 
    S( -56, -43), S( -26, -47), S(  -7, -57), S(  26, -67), 
    S( -86, -20), S(  -6, -52), S( -58, -20), S(   8, -58), 
    S( -52, -45), S( -86, -24), S( -33, -50), S(   4, -50), 
    S(  20, -14), S(  12, -12), S( -14,  -3), S(   6, -11), 
};

const int QueenPSQT32[32] = {
    S(  54,-135), S(   7,-107), S(  17,-124), S(  21, -89), 
    S(  10, -94), S(  35,-129), S(  38,-152), S(  19, -80), 
    S(  -2, -75), S(  28, -76), S(   0, -41), S(   2, -52), 
    S(   2, -58), S(   5, -23), S( -16, -26), S( -40,  41), 
    S( -45, -17), S( -37,  23), S( -60, -21), S( -82,  63), 
    S( -90,  21), S( -78,  -3), S( -89, -12), S( -67, -15), 
    S( -49,  18), S(-151,  77), S( -55, -25), S(-125,  54), 
    S( -62,  -4), S(  -1, -26), S( -19, -30), S( -42,  -8), 
};

const int KingPSQT32[32] = {
    S(  69,-111), S(  58, -61), S( -32,  10), S( -56,   0), 
    S(  48, -22), S( -19,  -2), S( -75,  45), S( -97,  49), 
    S(  16, -31), S(  23, -19), S(  34,  19), S( -16,  53), 
    S(  19, -58), S( 181, -64), S(  98,  17), S(   0,  64), 
    S(  15, -34), S( 204, -70), S( 112,  16), S(  25,  42), 
    S(  90, -82), S( 250, -68), S( 199, -25), S(  87, -20), 
    S(   2,-155), S(  90, -46), S(  58, -22), S(  14, -38), 
    S(   5,-248), S( 139,-144), S( -48, -62), S( -44, -47), 
};

/* Pawn Evaluation Terms */

const int PawnCandidatePasser[2][RANK_NB] = {
   {S(   0,   0), S( -46, -17), S( -20,  21), S( -32,  54), S( -14, 108), S(  61, 118), S(   0,   0), S(   0,   0)},
   {S(   0,   0), S( -28,  37), S(  -9,  39), S(   8,  80), S(  25, 150), S(  55, 106), S(   0,   0), S(   0,   0)},
};
...
const int PawnConnected32[32] = {
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0), 
    S(  -4, -13), S(  20,  -1), S(   1,   0), S(   8,  15), 
    S(  18,  -2), S(  43,  -7), S(  36,   5), S(  36,  19), 
    S(  12,  -4), S(  36,   4), S(  16,  15), S(  26,  27), 
    S(  14,  14), S(  33,  22), S(  40,  37), S(  53,  31), 
    S( 104,  35), S(  76,  84), S( 116,  93), S( 145, 106), 
    S( 224,  -5), S( 408,   9), S( 456,  62), S( 478,  93), 
    S(   0,   0), S(   0,   0), S(   0,   0), S(   0,   0), 
};
...
const int KnightBehindPawn = S(   7,  37);

const int KnightMobility[9] = {
    S(-151,-209), S( -70,-190), S( -44, -82), S( -20, -39), 
    S(  -1, -21), S(  11,   7), S(  26,  10), S(  43,   7), 
    S(  66, -24), 
};

/* Bishop Evaluation Terms */

const int BishopPair = S(  44, 122);

const int BishopRammedPawns = S( -18, -26);
...
const int BishopBehindPawn = S(   4,  34);

const int BishopMobility[14] = {
    S(-128,-291), S( -65,-181), S( -29,-103), S(  -9, -58), 
    S(  11, -38), S(  26, -13), S(  32,   5), S(  33,  15), 
    S(  32,  26), S(  39,  28), S(  39,  29), S(  75,   7), 
    S(  81,  29), S( 138, -40), 
};
...
const int RookOnSeventh = S( -11,  54);

const int RookMobility[15] = {
    S(-294,-225), S(-107,-219), S( -38,-109), S( -25, -36), 
    S( -25,  -2), S( -25,  24), S( -24,  42), S( -11,  47), 
    S(   2,  52), S(   7,  60), S(  12,  69), S(  23,  73), 
    S(  22,  82), S(  49,  60), S( 170, -33), 
};

/* Queen Evaluation Terms */

const int QueenMobility[28] = {
    S(-122,-526), S(-420,-774), S(-112,-401), S( -32,-383), 
    S(  -9,-275), S(  -4,-160), S(   4, -97), S(   2, -39), 
    S(  11, -32), S(  12,   5), S(  18,  13), S(  19,  42), 
    S(  23,  29), S(  26,  45), S(  21,  50), S(  12,  63), 
    S(  12,  61), S(  -6,  67), S(  -5,  62), S( -16,  51), 
    S(   1,   4), S(  30, -41), S(  38, -97), S(  47,-127), 
    S(  17,-152), S(  42,-203), S(-112, -79), S( -64,-127), 
};

/* King Evaluation Terms */

const int KingDefenders[12] = {
    S( -49,   8), S( -10,  -7), S(   3,   2), S(  17,   8), 
    S(  33,  10), S(  51,   8), S(  58,  -5), S(  26,   0), 
    S(  24,  12), S(  24,  12), S(  24,  12), S(  24,  12), 
};

const int KingPawnFileProximity[FILE_NB] = {
    S(  66,  55), S(  32,  39), S(   7,  22), S( -30, -27), 
    S( -35, -91), S( -32,-122), S( -30,-135), S( -22,-140), 
};

const int KingShelter[2][FILE_NB][RANK_NB] = {
  {{S( -21,   4), S(  25, -52), S(  38, -17), S(  24,   8), S(  13,   8), S(   1,   4), S(  -6, -66), S( -92,  36)},
   {S(  34, -17), S(  34, -34), S(   2, -10), S( -28,   6), S( -59,  30), S(-140, 135), S( 184, 164), S( -43,   3)},
   {S(  69, -11), S(  24, -18), S( -54,  13), S( -21, -16), S( -38,  -8), S( -23,   2), S(   0, 132), S( -19,  -5)},
   {S(   7,  19), S(  40, -23), S(   6, -22), S(  27, -43), S(  46, -71), S(-117,  10), S(-272, 104), S(  13, -15)},
   {S( -30,  13), S(   8, -10), S( -49,   1), S( -33,   9), S( -37, -11), S( -83,  -3), S(  66, -34), S(  -9,  -4)},
   {S(  87, -36), S(  39, -32), S( -40,   3), S( -22, -34), S(   9, -47), S(  34, -41), S(  82, -60), S( -44,   1)},
   {S(  48, -27), S(  -6, -29), S( -44,  -2), S( -38, -14), S( -58, -13), S( -73,  64), S(   0,  88), S( -17,   0)},
   {S( -17, -25), S(   8, -35), S(  11,   2), S(  -5,  22), S( -23,  31), S( -18,  74), S(-380, 174), S( -27,  24)}},
  {{S(   0,   0), S( -21, -46), S(  10, -37), S( -80,  32), S( -45,   4), S(   6,  84), S(-334, -16), S( -89,  19)},
   {S(   0,   0), S(  42, -43), S(  12, -16), S( -35,  -2), S(  -2, -24), S(  52, 131), S(-368,  -6), S( -70,   9)},
   {S(   0,   0), S(  50, -26), S(  -6, -18), S(  10, -36), S(  30, -14), S(-174,  94), S(-168,-146), S( -31,  -5)},
   {S(   0,   0), S( -10,  11), S(  -8,  -6), S( -36,   3), S( -54,   1), S(-198,  62), S(  14, -82), S( -31,  -4)},
   {S(   0,   0), S(  23,   0), S(  18, -13), S(  25, -24), S(  26, -52), S(-115,  30), S(-208,-122), S(   3,  -4)},
   {S(   0,   0), S(   8, -18), S( -40,  -3), S( -51, -12), S(  34, -48), S( -76,   6), S( 110,  76), S( -28,  -7)},
   {S(   0,   0), S(  38, -29), S(  18, -27), S( -19, -14), S( -51,  19), S( -18,  30), S(-112, -98), S( -52,  23)},
   {S(   0,   0), S(  19, -74), S(  34, -54), S( -36,  -9), S( -35,  36), S( -11,  40), S(-456,-110), S( -40,   5)}},

};

const int KingStorm[2][FILE_NB/2][RANK_NB] = {
  {{S(  -9,  55), S( 230, -25), S( -49,  48), S( -33,  14), S( -27,   5), S( -16,  -6), S( -32,  11), S( -45,   4)},
   {S(  -7,  97), S( 112,  15), S( -38,  43), S( -10,  21), S(  -8,  11), S(   8,  -6), S(  -3,   4), S( -19,   5)},
   {S(  15,  75), S(  31,  37), S( -44,  36), S( -21,  14), S(   4,   3), S(  12,   0), S(  18, -11), S(   6,   2)},
   {S(  -5,  49), S(  28,  32), S( -35,  13), S( -29,   1), S( -26,   1), S(  12, -22), S(   2, -16), S( -22,   6)}},
  {{S(   0,   0), S( -31, -32), S( -33,   0), S(  32, -30), S(  14, -11), S(   9, -34), S(  -4,  -1), S(  34,  58)},
   {S(   0,   0), S( -32, -69), S(  -2, -13), S(  71, -20), S(  -1,  -4), S(  26, -45), S( -14, -20), S( -34,   4)},
   {S(   0,   0), S( -56, -99), S( -51,  -9), S(  22, -21), S(   7,  -3), S( -14, -27), S( -26, -29), S( -22,  12)},
   {S(   0,   0), S(  -4, -36), S( -29, -32), S( -23,  -8), S(  -7, -13), S(   6, -49), S( 143, -23), S(  26,  40)}},
};
...
const int PassedPawn[2][2][RANK_NB] = {
  {{S(   0,   0), S( -63,  17), S( -92,  41), S(-132,  68), S( -14,  27), S( 144, -16), S( 311, 105), S(   0,   0)},
   {S(   0,   0), S( -56,  31), S(-100,  57), S(-125,  82), S( -30,  71), S( 175,  61), S( 368, 170), S(   0,   0)}},
  {{S(   0,   0), S( -37,  52), S( -88,  48), S(-121,  81), S( -14,  77), S( 163,  69), S( 498, 181), S(   0,   0)},
   {S(   0,   0), S( -55,  43), S( -85,  41), S(-112,  88), S(  -6,  88), S( 199, 169), S( 320, 447), S(   0,   0)}},
};
...
const int ThreatWeakPawn = S( -21, -47);
const int ThreatMinorAttackedByPawn = S( -88,-100);
const int ThreatMinorAttackedByMinor = S( -42, -65);
const int ThreatMinorAttackedByMajor = S( -39, -89);
const int ThreatRookAttackedByLesser = S( -77, -48);
const int ThreatQueenAttackedByOne = S( -56, -87);
const int ThreatOverloadedPieces = S( -13, -22);
const int ThreatByPawnPush = S(  25,  34);
